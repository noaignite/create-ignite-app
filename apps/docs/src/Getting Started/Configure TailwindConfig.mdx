...
```bash
pnpm install -DW @noaignite/tailwind-typography
```
...
import {
  ...
  typography,
} from "@noaignite/tailwind-typography";
...
@layer base {
  ...
  [data-color-scheme="light"] {
    ...
    --palette-action-disabledBg: rgba(
      theme("colors.common.blackChannel") / 0.12
    );
  }
  ...
  [data-color-scheme="dark"] {
    ...
    --palette-action-disabledBg: rgba(
      theme("colors.common.whiteChannel") / 0.12
    );
  }
